...
int compare(const std::variant<int, Packet>& first, const std::variant<int, Packet>& second) {
  if (std::holds_alternative<int>(first) && std::holds_alternative<int>(second)) {
    // std::cout << "Both ints: " << std::get<int>(first) << " " << std::get<int>(second) << '\n';
...
class Solution {
 public:
  ...
  int part2(const std::string& pathToInput) {
    ...
    packets.push_back(firstDivider), packets.push_back(secondDivider);
    std::string newpacket;
    while (input >> newpacket) packets.push_back(Packet::fromString(newpacket));
    std::sort(packets.begin(), packets.end(),
              [](const auto& first, const auto& second) { return compare(first, second) >= 0; });
    auto firstDividerPtr = std::find_if(packets.begin(), packets.end(),
                                        [&](const auto& packet) { return compare(packet, firstDivider) == 0; });
    auto secondDividerPtr = std::find_if(packets.begin(), packets.end(),
                                         [&](const auto& packet) { return compare(packet, secondDivider) == 0; });

    std::cout << "Index first: " << std::distance(packets.begin(), firstDividerPtr)
              << " index second: " << std::distance(packets.begin(), secondDividerPtr) << '\n';
    return std::distance(packets.begin(), firstDividerPtr) * std::distance(packets.begin(), secondDividerPtr);
  }
};
