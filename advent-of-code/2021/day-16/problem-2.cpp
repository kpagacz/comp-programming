...
struct Packet {
  ...
  uint64_t get_value() {
    ...
    switch(id) {
      ...
      case 0:
        answer = 0;
        std::for_each(subpackets.begin(), subpackets.end(), [&](auto p) { answer += p->get_value(); });
        break;
      case 1:
        answer = 1;
        std::for_each(subpackets.begin(), subpackets.end(), [&](auto p) { answer *= p->get_value(); });
      ...
      case 2:
        ...
        std::for_each(subpackets.begin(), subpackets.end(), [&](auto p) { if (p->get_value() < answer) answer = p->get_value(); });
        break;
      case 3:
        answer = 0;
        std::for_each(subpackets.begin(), subpackets.end(), [&](auto p) { if (p->get_value() > answer) answer = p->get_value(); });
        break;
      case 5:
        answer = (int)(subpackets[0]->get_value() > subpackets[1]->get_value());
        break;
      case 6:
        answer = (int)(subpackets[0]->get_value() < subpackets[1]->get_value());
        break;
...
int main(int argc, char* argv[]) {
  ...
  while(std::cin >> in) {
    ...
    std::cout << "Value of the packet: " << p->get_value() << '\n';
  }
}
