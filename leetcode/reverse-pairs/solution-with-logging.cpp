...
class Solution {
 public:
  int reversePairs(std::vector<int>& nums) {
    ...
    // if (debug) {
    //   std::copy(sortedIndexes.begin(), sortedIndexes.end(), std::ostream_iterator<int>(std::cout, " "));
    //   std::cout << '\n';
    // }
    ...
    for (int i = nums.size() - 1; i >= 0; --i) {
      // if (debug) std::cout << "num[" << i << "]: " << nums[i] << '\n';
      const int searchedValue = (nums[i] >> 1) + (nums[i] % 2 == 1 || nums[i] % 2 == -1);
      // if (debug) std::cout << "Searched value: " << searchedValue << '\n';
      ...
      // if (debug && greaterEqualNum != sortedIndexes.end())
      //   std::cout << "Greater equal num: " << nums[*greaterEqualNum]
      //             << " index in si: " << std::distance(sortedIndexes.begin(), greaterEqualNum) << '\n';
      answer += bit.sum(std::distance(sortedIndexes.begin(), greaterEqualNum));
      // if (debug) std::cout << "Answer: " << answer << '\n';
      bit.add(indexToSortedIndexMap[i], 1);
      // if (debug) bit.print(std::cout);
    }
